...
async function listLayouts(registry, cwd = process.cwd()) {
    const stdout = await (0, exec_1.runGenerator)(cwd, '@blueprint-platform/foundation:layout', ['--list'], registry);
    return parseListOutput(stdout);
}
/** Every UI-kit component name available via `components:ui` (see B2). */
async function listComponents(registry, cwd = process.cwd()) {
    const stdout = await (0, exec_1.runGenerator)(cwd, '@blueprint-platform/components:ui', ['--list'], registry);
    return parseListOutput(stdout);
}
/**
...
const MODULES = [
    {
        name: 'auth',
        description: 'JWT or session authentication, with login/signup/forgot-password/change-password forms and its own layout.',
    },
    { name: 'rbac', description: 'Roles & permissions module, works standalone.' },
    {
        name: 'user-management',
        description: 'User list + add/edit/details/change-role/deactivate, works standalone.',
    },
];
async function listModules(registry) {
    return [...MODULES];
}
