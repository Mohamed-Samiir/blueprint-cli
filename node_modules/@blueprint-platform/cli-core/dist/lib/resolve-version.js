...
exports.resolveFoundationVersion = resolveFoundationVersion;
...
async function resolveFoundationVersion(channel, registry) {
    const version = await (0, exec_1.npmView)('@blueprint-platform/foundation', [`dist-tags.${channel}`], registry);
    if (!version) {
        throw new Error(`resolveFoundationVersion: no "${channel}" dist-tag found for @blueprint-platform/foundation` +
